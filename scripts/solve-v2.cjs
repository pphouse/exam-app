...
const questions = JSON.parse(fs.readFileSync(`${DIR}/rewritten47v2.json`, 'utf8'));
...
(async()=>{
  ...
  fs.writeFileSync(`${DIR}/solve-v2-result.json`, JSON.stringify(results, null, 2));
  const total=results.reduce((s,r)=>s+r.correctCount,0);
  console.log('OVERALL=' + Math.round(total/(results.length*TRIALS)*100) + '% (' + total + '/' + results.length*TRIALS + ')');
  const byChap={};
  ...
  Object.entries(byChap).sort((a,b)=>a[0].localeCompare(b[0])).forEach(([ch,d])=>console.log(ch+': '+Math.round(d.c/d.t*100)+'%'));
  // 間違えた問題一覧
  const missed = results.filter(r=>r.accuracy<100);
  console.log('\n正解率<100%: ' + missed.length + '問');
  missed.forEach(r=>console.log(' ',r.qid, r.chapter, r.accuracy+'%', r.answers));
})();
