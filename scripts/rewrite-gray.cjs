// グレーゾーン版リライト
// 全選択肢を「部分的に正しい」状態にし、最も適切な1つを選ばせる
// 出力: rewritten47v3.json
...
let existing = {};
try {
  const prev = JSON.parse(fs.readFileSync(`${DIR}/rewritten47v3.json`, 'utf8'));
...
- 「正しい概念を含むが、優先順位や文脈がずれている」
  例：ハルシネーション対策として「RAG導入」は有効だが「最も重要な対策」ではない
- 「条件付きで正しいが、無条件に正しいとは言えない」
  例：「ファインチューニングで最新情報に対応できる」→条件によっては正しいが、一般論ではRAGの方が適切
- 「正しい事実だが、問いの焦点とズレている」
  例：「オープンソースモデルはデータを外部送信しない」→正しいが「最も重要な選定基準」かは別の話
- 「一方向では正しいが例外がある」
  例：「ハルシネーションはRAGで完全に防げる」→×、「ハルシネーションはRAGで低減できる」→△
...
- why_wrongには「完全に誤りではないが〜という点で最も適切ではない」と明記すること
...
async function rewriteOne(q) {
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      model: MODEL, max_tokens: 1200, temperature: 0.7,
      system: SYSTEM,
...
(async () => {
  ...
  for (let i = 0; i < todo.length; i++) {
    ...
    fs.writeFileSync(`${DIR}/rewritten47v3.json`, JSON.stringify(sorted, null, 2));
