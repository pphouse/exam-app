...
const questions = JSON.parse(fs.readFileSync(`${DIR}/rewritten47v3.json`, 'utf8'));
...
(async()=>{
  ...
  fs.writeFileSync(`${DIR}/solve-v3-result.json`, JSON.stringify(results, null, 2));
  const total=results.reduce((s,r)=>s+r.correctCount,0);
  console.log('v3 OVERALL=' + Math.round(total/(results.length*TRIALS)*100) + '% (' + total + '/' + results.length*TRIALS + ')');
  const byChap={};
